...
初回テスト実行時に検出された不具合（×）について、原因の特定とコードの修正を行い、再テストにてすべての安全性が確認されました（再テスト時はすべて「○」に改善済み）。

#### ① 検出された不具合と修正内容

* **#10：フリック中にブラウザ外へ離脱した際に意図しない入力が発生する問題（異常系）**
  * **現象**: 入力パッド内でドラッグを開始し、ブラウザのウィンドウ外までマウスを動かして離しても、文字入力が実行されてしまう。
  * **原因**: ポインターキャプチャ（`setPointerCapture`）の仕様上、ブラウザ外でも座標検知が継続し、画面端の座標を基にフリック方向が計算されてしまっていた。
  * **修正**: `inputPad.getBoundingClientRect()` を導入。マウスを離した最終座標が「入力パッドの枠外」である場合は、入力を確定させずに安全に処理をキャンセル（`return`）する実装へ変更した。これにより、外枠に大きく外したりブラウザ外に出したりした操作はすべて安全に無視されるようになった。

* **#11：右クリックでのフリック操作で文字が入力されてしまう問題（異常系）**
  * **現象**: マウスの右ボタンを押しながらドラッグした場合でも、通常のフリック入力として処理されてしまう。
  * **原因**: `pointerdown` イベント内で、クリックされたボタンの種別（主ボタンか副ボタンか）を判別するガード句が不足していた。
  * **修正**: イベント発生時に `if (e.button !== 0) return;` を追加し、左クリック（主ボタン）以外の操作を完全に除外した。また、右クリック時のコンテキストメニューが操作を阻害しないよう `contextmenu` イベントの発生を抑止する処理も一律で追加した。

#### ② テスト対象外（仕様）に関する判断

* **#24：テキストエリア内の文字列を選択した状態での操作（境界入力）**
  * **判断理由**: 本アプリは、入力パッドでのドラッグ操作中に背景の文字が青くハイライトされる視覚的なノイズを防ぐため、CSSで `user-select: none;`（テキスト選択禁止）を意図的に指定している。そのため、テキストエリア内の文字を範囲選択すること自体が「仕様上発生し得ない」ため、本ケースは「検証対象外（ー）」とした。

---

### 11. SHIFT良質テスト3視点に基づく品質保証の振り返り

本実習を通じて、以下の3視点からアプリの品質が担保されていることを確認しました。

1. **仕様視点（要件どおりの振る舞いか）**
   * ボタンクリックおよびフリックによる「あ・い・う・え・お」の5方向入力、1文字削除、全クリアなど、PBL①で定義した基本要件がすべて正確に動作することを確認した。
2. **実装視点（異常な入力・境界値で壊れないか）**
   * テキストエリアが空欄（0文字）の状態で削除やクリアを押す「境界入力」や、ドラッグ中のブラウザ外離脱・右クリックなどの「異常系」の操作に対しても、アプリがフリーズやクラッシュを起こさず安全に処理をスルーまたはキャンセルできる堅牢性を実装した。
3. **利用視点（実際の利用者がつまずかないか）**
   * 同一文字の高速連打時にも処理落ち（ラグ）が発生せず、文字が枠を越えるほどの長文入力（50文字以上）を行った際にも表示崩れが起きずスクロールで追従するなど、実際のユーザーがストレスなく利用できる操作性を確認した。
